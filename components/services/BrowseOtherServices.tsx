...
export function BrowseOtherServices({ current }: { current: ServicePageData }) {
  ...
  return (
    <section className="page-shell py-20 md:py-28">
      <h2 className="display border-b border-olive/25 pb-5 text-3xl">Explore More Services</h2>
      <div className="mt-12 grid gap-6 md:grid-cols-3">
        {alternatives.map((service, index) => (
          <Link
            key={service.slug}
            href={`/services/${service.slug}`}
            className={`group relative block overflow-hidden bg-bone ${index === 1 ? "md:mt-16" : ""}`}
          >
